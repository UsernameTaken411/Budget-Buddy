import { Cloud, Landmark, Sparkles, WalletCards, WifiOff } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#090d0c] text-slate-100">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-white/[0.07] bg-[#0b100f]/95 p-5 backdrop-blur-xl lg:flex">
        <div className="flex items-center gap-3 px-2 py-3">
          <span className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-400 text-[#07100d]"><Landmark /></span>
        ...
        <NavLink to="/budgets" className="flex items-center gap-3 rounded-xl bg-emerald-400/10 px-3 py-3 text-sm font-semibold text-emerald-300">
          <WalletCards size={17} />Budgets
        </NavLink>
        <div className="mt-auto rounded-2xl border border-emerald-400/10 bg-emerald-400/[0.04] p-4">
          <Sparkles className="mb-3 text-emerald-300" size={18} />
          <p className="text-sm font-bold">Money, without the mystery.</p>
          <p className="mt-1 text-xs leading-5 text-slate-500">Set limits and keep spending intentional.</p>
        </div>
      ...
      <nav className="fixed inset-x-3 bottom-3 z-40 rounded-2xl border border-white/10 bg-[#111715]/95 p-2 lg:hidden">
        <NavLink to="/budgets" className="flex flex-col items-center gap-1 rounded-xl bg-emerald-400/10 px-3 py-2 text-[11px] font-semibold text-emerald-300"><WalletCards size={18} />Budgets</NavLink>
      </nav>
    </div>
  );
}
